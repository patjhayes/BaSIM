...
def build_model(basin_length, basin_width, basin_depth, gw_clearance, hk, sy, ts1_data):
    ...
    sim = flopy.mf6.MFSimulation(
        sim_name=model_name, 
        version="mf6", 
        exe_name=r"..\..\Documents\mf6.6.2_win64\bin\mf6.exe",
        sim_ws=model_ws
    )
    ...
    initial_head = top - gw_clearance - 2  # Water table 2m below clearance
    ...
    # Lake package data
    packagedata = [(lake_id, strt, nlakes)]
    
    # Create lake-aquifer connections
    connectiondata = []
    iconn = 0
    
    for row in range(row_start, row_end):
        for col in range(col_start, col_end):
            # Connect to layer 0 (horizontal connection at lake bottom)
