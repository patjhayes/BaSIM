...
def build_lak_model_step31(basin_length, basin_width, basin_depth, basin_level, gw_level, hk, sy):
    ...
    # ============================================
    # LAK PACKAGE SETUP - ULTRA SIMPLE
    # ============================================
    
    print("\n" + "="*40)
    print("🏞️ LAK PACKAGE SETUP (ULTRA SIMPLE)")
    print("="*40)
    ...
    # Build connection data - use 0-based lake indexing
    lak_connectiondata = []
    iconn = 0
    
    for (row, col) in basin_cells:
        # Use 0-based lake indexing throughout
    ...
    # Period data - DICTIONARY FORMAT WITH EXPLICIT STRESS PERIOD
    test_inflow = 0.0001  # 0.1 L/s
    
    # Try dictionary format: {stress_period: [(lakeno, setting, value)]}
    lak_perioddata = {0: [(0, 'RATE', test_inflow)]}
    ...
    # Create LAK package - MINIMAL OPTIONS
    print(f"\n🏗️ Creating LAK package with minimal options...")
    
    try:
        lak = flopy.mf6.ModflowGwflak(
            gwf,
            nlakes=1,
            noutlets=0,  # NO OUTLETS
