...
new_mvr_func = '''def create_mvr_package(
    basin_cells,
    stress_periods,
    model_dir,
    model_name,
...
# In PowerShell, \\n becomes \n inside here string. 
# So in Python, the string contains literal \n which is fine.
# But we need it to be string characters '\n' in the python source file, so we replace literal newlines with '\n'.
new_mvr_func = new_mvr_func.replace('\n', '\r\n')

content = re.sub(mvr_pattern, new_mvr_func, content, flags=re.DOTALL)
# Now we must ensure the f.write strings have literal \n inside them!
# Wait, the f.write strings inside new_mvr_func have \n. If we just leave them, they are fine.

# Wait, we need to convert \\n back to \n in the python source text if it was escaped.
content = content.replace('f.write("BEGIN DIMENSIONS\\n")', 'f.write("BEGIN DIMENSIONS\\n")')
